...
class generatorWithSubstitutions():
    ...
    def run(self):
        ...
        for iii in range(len(self.molin_ok)):
            ...
            molFile = ""
            outmol = []
            fp1 = AllChem.GetMorganFingerprintAsBitVect(Chem.MolFromSmiles(self.molin_ok[iii]),2)
            ...
            # Not today!
                ...
                try:
                    ...
                    try:
                        ...
                        SA = 10
                    threshhold = float(SAScore.calculateScore(Chem.MolFromSmiles(self.molin_ok[iii]))) + 1.0
                ...
                if valid ==True:
                    ...
                    if answer1 not in outmol:
                        if  answer1 != self.molin_ok[iii] and dist==0 and SA<threshhold:
                            qed = QED.default(Chem.MolFromSmiles(answer1))
                            fp2 = AllChem.GetMorganFingerprintAsBitVect(Chem.MolFromSmiles(answer1),2)
                            similarity = DataStructs.FingerprintSimilarity(fp1,fp2)
                            valids += 1
                            molFile += answer1 + ";"+ str(qed) +";"+str(SA)+";"+str(similarity) +"\n"
            ...
            fh = open(thisPath + "/" + str(iii+1) + ".smi", "w")
            fh.write(outfff)
            fh.close()
...
params={"name": sys.argv[1],
            "vocabularyPath": scriptPath + "/models/training_full_dataset_c12.voc",
